...
class TicketService {
  ...
  ticket(guildId, ticketId) {
    const ticket = this.repository.get(guildId, ticketId);
  ...
  limits(guildId, userId, categoryId, excludeId) {
    const tickets = this.repository.list(guildId).filter(ticket => ticket.creatorUserId === userId && ticket.id !== excludeId);
  ...
  async create({ guildId, userId, channelId, categoryId, subject, description }) {
    return this.exclusive(guildId, async () => {
      ...
      let ticket = this.repository.list(guildId).find(item => item.creatorUserId === userId && item.categoryId === categoryId && !item.initialized && item.status === S.OPEN);
      if (!ticket) {
        this.limits(guildId, userId, categoryId);
        await this.adapter.validateStructure(config);
        ticket = this.repository.create({ guildId, guildName: await this.adapter.guildName(guildId),
      ...
      this.event(ticket, E.CREATED, userId);
      this.repository.save(ticket);
  ...
  async claim({ guildId, userId, ticketId, channelId }) {
    return this.exclusive(guildId, async () => {
      const ticket = this.ticket(guildId, ticketId);
  ...
  async reopen({ guildId, userId, ticketId, channelId, cycle }) {
    return this.exclusive(guildId, async () => {
      ...
      if (!ticket.reopening) {
        this.limits(guildId, ticket.creatorUserId, ticket.categoryId, ticket.id);
