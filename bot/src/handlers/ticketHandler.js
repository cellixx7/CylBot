...
async function report(interaction, error) {
  ...
  const statusCode = controlled ? error.statusCode : details.rateLimitSource === 'discord' ? 429 : 500;
  logger[statusCode < 500 ? 'warn' : 'error']('ticket.interaction_failed', { ...ticketLogContext(), statusCode, ...details });
  const payload = privateReply(controlled ? error.message : details.rateLimitSource === 'discord'
    ? 'O Discord limitou temporariamente esta operação. Aguarde e tente novamente.'
    : 'Não foi possível concluir a operação. Os dados salvos foram preservados; tente novamente.');
  try {
    if (interaction.deferred || interaction.replied) await interaction.followUp(payload);
    else await interaction.reply(payload);
  } catch (replyError) {
    logger.warn('ticket.interaction_response_failed', { ...ticketLogContext(), ...errorDetails(replyError) });
  }
}
async function startTicketSetup(interaction, services = interaction.client.services) {
  return runTicketOperation(interaction, { action: 'setup.begin' }, async () => {
    try {
...
async function handleTicketInteraction(interaction, services) {
  ...
  return runTicketOperation(interaction, interactionDetails(interaction.customId), () => executeTicketInteraction(interaction, services));
}
async function executeTicketInteraction(interaction, services) {
  try {
    ...
    if ((match = customId.match(new RegExp(`^ticket:setup:(start|cancel|role|auto|existing|panel|log|category|defaults|custom|confirm):(${UUID})$`)))) {
      ...
      if (action === 'remove') {
        ...
        if (action === 'reopen') setTicketContext(ticket);
        setTicketStage('interaction.response');
