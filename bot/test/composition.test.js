...
test('composition root compartilha providers, services e repository sem iniciar operações externas', () => {
  ...
  assert.equal(services.ticketAI.context.messages, services.ticketMessages);
  assert.equal(services.ticketMessages.adapter, services.tickets.adapter);
  assert.equal(services.tickets.transcripts.messages, services.ticketMessages);
