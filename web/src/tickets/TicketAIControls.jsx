...
export default function TicketAIControls({ status, active, busy, result, error, onSuggest, onPause, onResume, onConfigure }) {
  ...
  return <section className="ticket-ai" aria-label="Assistente IA">
    <div><h3>Assistente IA</h3><p>{status.assistantName ? `${status.assistantName} · ` : ''}{state}</p></div>
    ...
    {status.canConfigure && <button className="ticket-ai-configure" disabled={Boolean(busy)} onClick={onConfigure}>Configurar IA do servidor</button>}
