export default function GuildCard({ guild }) {
  return (
    <article className="guild-card">
      ...
      <span className={`guild-status ${guild.botInstalled ? 'guild-installed' : ''}`}>{guild.botInstalled ? 'CylBot instalado' : 'CylBot não instalado'}</span>
      ...
      <div className="guild-action">
        {guild.botInstalled && guild.canManage ? <a className="landing-cta" href={`#/dashboard/${guild.id}`}>Gerenciar <span aria-hidden="true">↗</span></a>
